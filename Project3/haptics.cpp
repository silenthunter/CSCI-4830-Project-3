...
void HapticsClass::forceDirection(Vector3 unitVector, double magnitude){
	...
	//for (int i = 9; i > 1; i--){
	//	for (int j = 0; j < 3; j++){
	//	unitVectorStorage[i][j] = unitVectorStorage[i-1][j];
	//	}}
	//	for (int j = 0; j < 3; j++){
	//	unitVectorStorage[0][j] = unitVectorGraphic[j];
	//	}
	//for (int i = 9; i > 0; i--){
	//	for (int j = 0; j < 3; j++){
	//			unitVectorStorage[i][j] = (unitVectorStorage[i][j]/10);
	//	}
	//}
	//for (int j = 0; j < 3; j++){
	//	for (int i = 9; i > 0; i--){	
	//	AVGUNITVECTOR[j] += unitVectorStorage[i][j];
	//	}
	//}
...
{ 
	...
	double filter = 0;
		m_forceServo[0] = filter * m_forceServo[0]+ ( (1-filter) * unitVectorGraphic[0] * pow(magnitudeVectorGraphic,2) * 100);//  * pow(5,magnitudeVectorGraphic);
		m_forceServo[1] = filter * m_forceServo[1] + ( (1-filter) * unitVectorGraphic[1] * pow(magnitudeVectorGraphic,2) * 100);//  * pow(5,magnitudeVectorGraphic);
